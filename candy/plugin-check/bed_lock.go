...
// podman store, so the second run reclaimed the first run's containers. Measured (check run
// 2026.255.2323): a peer lane's run SIGTERMed the pod container `charly-check-githubrunner-pod`
// at 01:29:25, after which all 67 container-exec probes read `container state improper` /
// exit-125 — the run was green up to that instant and had no way to see the peer.
//
