...
// `charly deploy del <entity> --assume-yes`. The session's teardown destroyed only the first
...
// match over `charly-deploy-del-*` would reach a CONCURRENT bed's armed timers and cancel the
...
//
// Reads through the PAIRED LOADER (loaderkit.LoadHostDeployConfigViaExecutor), the counterpart of
// the writer that persists these registrations — never a hand-rolled yaml.Unmarshal. That is not a
// style preference: DeployConfig.Deploy is tagged `yaml:"deploy"`, but SaveDeployConfig writes
// entity-name keys carrying node-form bodies and LoadDeployConfig delegates to the unified loader
...
func ephemeralTimerUnitsIn(dc *deploykit.DeployConfig) []string {
	if dc == nil {
		return nil
	}
	var units []string
